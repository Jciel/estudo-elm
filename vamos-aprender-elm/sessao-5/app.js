...
var author$project$Avatar$model = elm$core$Maybe$Nothing;
var author$project$Main$Comment = F5(
	function (author, content, email, date, saved) {
		return {author: author, content: content, date: date, email: email, saved: saved};
	});
var author$project$Main$Model = F3(
	function (comments, form, loaded) {
		return {comments: comments, form: form, loaded: loaded};
	});
...
var author$project$Main$initialModel = A3(
	...
	_List_Nil,
	A5(author$project$Main$Comment, '', '', author$project$Avatar$model, '', false),
	false);
var author$project$Main$ApiCall = function (a) {
	return {$: 'ApiCall', a: a};
};
...
var author$project$Main$decodeComment = A2(
...
var author$project$Main$decode = A2(
	elm$json$Json$Decode$field,
	'comments',
	elm$json$Json$Decode$list(author$project$Main$decodeComment));
...
var author$project$Main$loadComments = elm$http$Http$get(
	{
		expect: A2(elm$http$Http$expectJson, author$project$Main$ApiCall, author$project$Main$decode),
		url: 'http://vamosaprenderelm.herokuapp.com/api/comments'
	});
var author$project$Main$init = function (flags) {
	return _Utils_Tuple2(author$project$Main$initialModel, author$project$Main$loadComments);
};
var author$project$Main$resetForm = function (model) {
	return _Utils_update(
		model,
		{
			form: A5(author$project$Main$Comment, '', '', author$project$Avatar$model, '', false)
		});
};
var author$project$Main$ApiPost = function (a) {
	return {$: 'ApiPost', a: a};
};
...
var author$project$Main$saveComment = function (comment) {
	var email = function () {
		var _n0 = comment.email;
		if (_n0.$ === 'Just') {
			var emaill = _n0.a;
			return emaill;
		} else {
	...
	var data = elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'author',
				elm$json$Json$Encode$string(comment.author)),
				_Utils_Tuple2(
				'content',
				elm$json$Json$Encode$string(comment.content)),
				_Utils_Tuple2(
				'email',
	...
	return elm$http$Http$post(
		{
			...
			expect: A2(elm$http$Http$expectJson, author$project$Main$ApiPost, author$project$Main$decodeComment),
			url: 'http://vamosaprenderelm.herokuapp.com/api/comments/'
		});
};
var author$project$Main$updateCommentStatus = F2(
	function (_new, current) {
		return (_Utils_eq(current.author, _new.author) && _Utils_eq(current.content, _new.content)) ? _Utils_update(
			current,
			{saved: true}) : current;
	});
...
var author$project$Main$updateCommentsStatus = F2(
	function (newComment, comments) {
		return A2(
			elm$core$List$map,
			author$project$Main$updateCommentStatus(newComment),
			comments);
	});
...
var author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'LoadComments':
				return _Utils_Tuple2(model, author$project$Main$loadComments);
			case 'ApiSuccess':
				var comments = msg.a;
				return _Utils_Tuple2(
					author$project$Main$resetForm(model),
					elm$core$Platform$Cmd$none);
			case 'ApiCall':
				if (msg.a.$ === 'Ok') {
					var comments = msg.a.a;
					var modell = _Utils_update(
						model,
						{comments: comments, loaded: true});
					return _Utils_Tuple2(modell, elm$core$Platform$Cmd$none);
				} else {
					var error = msg.a.a;
			...
			case 'ApiPost':
				if (msg.a.$ === 'Ok') {
					var comment = msg.a.a;
					var newComments = A2(author$project$Main$updateCommentsStatus, comment, model.comments);
					var modell = _Utils_update(
						model,
						{comments: newComments, loaded: true});
					return _Utils_Tuple2(modell, elm$core$Platform$Cmd$none);
				} else {
					var error = msg.a.a;
					return _Utils_Tuple2(model, elm$core$Platform$Cmd$none);
			...
			case 'RessetForm':
				return _Utils_Tuple2(
					...
					elm$core$Platform$Cmd$none);
			case 'PostComment':
				var content = elm$core$String$trim(model.form.content);
				var author = elm$core$String$trim(model.form.author);
				var modell = (elm$core$String$isEmpty(author) && elm$core$String$isEmpty(content)) ? model : _Utils_update(
					model,
					{
						comments: A2(
				...
				return _Utils_Tuple2(
					author$project$Main$resetForm(modell),
					author$project$Main$saveComment(model.form));
			case 'UpdateAuthor':
				var author = msg.a;
				var form = model.form;
			...
			default:
				...
				var newForm = _Utils_update(
					form,
					{
						email: elm$core$Maybe$Just(email)
					});
				return _Utils_Tuple2(
					_Utils_update(
						model,
...
var author$project$Main$PostComment = {$: 'PostComment'};
...
var author$project$Main$pluralize = F2(
	function (name, count) {
...
var author$project$Avatar$view = function (email) {
	if (email.$ === 'Just') {
		var value = email.a;
		var hash = truqu$elm_md5$MD5$hex(value);
		...
		return A2(
			...
			_List_fromArray(
				[
					elm$html$Html$Attributes$src('https://www.gravatar.com/avatar?d=mm')
				]),
			_List_Nil);
	}
};
var author$project$Main$AvatarMessage = function (a) {
	return {$: 'AvatarMessage', a: a};
};
...
var author$project$Main$teste = function (comment) {
	var posixResult = rtfeldman$elm_iso8601_date_strings$Iso8601$toTime(comment.date);
	if (posixResult.$ === 'Ok') {
		var posix = posixResult.a;
		return A3(
...
var author$project$Main$viewComment = function (comment) {
	var date = author$project$Main$teste(comment);
	var commentClass = comment.saved ? '' : 'saving';
	var avatar = A2(
		elm$html$Html$map,
		function (msg) {
			return author$project$Main$AvatarMessage(msg);
		},
		author$project$Avatar$view(comment.email));
	return A2(
		elm$html$Html$li,
		_List_fromArray(
			[
				elm$html$Html$Attributes$class(commentClass)
			]),
		_List_fromArray(
			[
				avatar,
...
var author$project$Main$view = function (model) {
	if (model.loaded) {
		var count = elm$core$List$length(model.comments);
		var title = _Utils_ap(
			elm$core$String$fromInt(count),
			A2(author$project$Main$pluralize, ' Comentário', count));
		return A2(
			elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
				[
					...
					A2(elm$core$List$map, author$project$Main$viewComment, model.comments)),
					A2(
					elm$html$Html$form,
					_List_fromArray(
						[
							elm$html$Html$Events$onSubmit(author$project$Main$PostComment)
						]),
					_List_fromArray(
						[
							A2(
								...
								[
									...
									_List_fromArray(
										[
											...
											elm$html$Html$Attributes$value(model.form.author)
										]),
									_List_Nil),
									A2(
									elm$html$Html$label,
									...
									_List_fromArray(
										[
											...
											A2(elm$core$Maybe$withDefault, '', model.form.email))
										]),
									_List_Nil)
								])),
							A2(
								...
								[
									...
									_List_fromArray(
										[
											...
											elm$html$Html$Attributes$value(model.form.content)
										]),
									_List_Nil)
								])),
							A2(
